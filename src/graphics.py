...
photos_segmented_path = str(
    pathlib.Path(__file__).parent.parent / "photos_segmented_textures"
)


def get_texture(
    tex_name: str, rng=None, segment: bool = False, style: str = None
) -> "Texture":
    ...
    if path not in Texture.tex_cache:
        Texture.tex_cache[path] = Texture(
            load_texture(oldpath, segment), tex_name=tex_name, rng=rng, style=style
        )

    return Texture.tex_cache[path]


def get_photos_segmented_texture(
    tex_name: str, rng=None, segment: bool = False, style: str = None
) -> "Texture":
    paths = [f"{photos_segmented_path}/{tex_name}.png"]

    # if rng:
    #     logger.warning("rng not supported for get_photos_segmented_texture")
    ...
    if path not in Texture.tex_cache:
        Texture.tex_cache[path] = Texture(
            load_texture(oldpath, False), tex_name=tex_name, rng=rng, style=style
        )
...
class Texture:
    ...
    def __init__(self, tex, tex_name, rng, style):
    ...
    def bind(self, segment=False):
        if segment:
            if "photos" in self.style:
                self = get_photos_segmented_texture(self.tex_name, self.rng, True)
            else:
...
def load_texture(
    tex_path: str, segment: bool = False, segment_into_color=None, resize_factor=1
):
    ...
    if resize_factor == 1:
        ...
        # check that resize factor is a number multiple of 2 or a number such that 1/resize_factor is a number multiple of 2
        assert resize_factor % 2 == 0 or np.isclose(1 / resize_factor % 2, 0), (
            "resize factor for the texture should be a number multiple of 2 or"
            " a number such that 1/resize_factor is a number multiple of 2"
        )

        im = cv2.imread(tex_path, cv2.IMREAD_UNCHANGED)
        im = cv2.resize(
            im, None, fx=resize_factor, fy=resize_factor, interpolation=cv2.INTER_AREA
        )
